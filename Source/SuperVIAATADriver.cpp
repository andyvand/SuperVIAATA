...
#include "SuperVIAATADriver.h"

#define super IOPCIATA
OSDefineMetaClassAndStructors( SuperVIAATADriver, IOPCIATA )
...
bool SuperVIAATADriver::start( IOService * provider )
{
	
	IOLog("SuperVIAATA for Snow Leopard by c0rk007hck\n");
    ...
    fProvider = OSDynamicCast( SuperVIAATAChannel, provider );
    if ( fProvider == 0 )
        goto fail;
    ...
    IOLog("%s: VIAx64 %s %s (CMD 0x%x, CTR 0x%x, IRQ %u, BM 0x%x)\n", getName(),
          fProvider->getHardwareName(),
		  fProvider->getControllerName(),
          (unsigned int)fProvider->getCommandBlockAddress(),
          (unsigned int)fProvider->getControlBlockAddress(),
          (unsigned int)fProvider->getInterruptVector(),
          (unsigned int)fBMBaseAddr);
...
void SuperVIAATADriver::stop( IOService * provider )
{
	FEDE_LOG("FEDE - %s: %s( %p, %p )\n", getName(), __FUNCTION__, this, provider);
    PMstop();
    super::stop( provider );
...
void SuperVIAATADriver::free( void )
{
...
IOWorkLoop * SuperVIAATADriver::getWorkLoop( void ) const
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
    return fWorkLoop;
}
...
IOReturn SuperVIAATADriver::synchronousIO( void )
{
	FEDE_LOG("FEDE - %s: %s( %p )\n", getName(), __FUNCTION__, this);
    IOReturn ret;
...
bool SuperVIAATADriver::getBMBaseAddress( UInt32   channel,
                                          UInt16 * baseAddr )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
    UInt32 bmiba;
...
void SuperVIAATADriver::resetBusTimings( void )
{
    DEBUG_LOG("%s::%s( %p )\n", getName(), __FUNCTION__, this);
...
bool SuperVIAATADriver::configureTFPointers( void )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
    DEBUG_LOG("%s::%s( %p )\n", getName(), __FUNCTION__, this);
...
bool SuperVIAATADriver::interruptFilter( OSObject * owner,
                                         IOFilterInterruptEventSource * src )
{
    SuperVIAATADriver * self = (SuperVIAATADriver *) owner;
...
void SuperVIAATADriver::interruptOccurred( OSObject *               owner,
...
UInt32 SuperVIAATADriver::scanForDrives( void )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
    UInt32 unitsFound;
...
IOReturn SuperVIAATADriver::provideBusInfo( IOATABusInfo * infoOut )
{
...
IOReturn SuperVIAATADriver::getConfig( IOATADevConfig * configOut,
                                       UInt32           unit )
{
...
IOReturn SuperVIAATADriver::selectConfig( IOATADevConfig * configRequest,
                                          UInt32           unit )
{
    DEBUG_LOG("%s::%s( %p, %p, %ld )\n", getName(), __FUNCTION__,
...
void SuperVIAATADriver::selectTimingParameter( IOATADevConfig * configRequest,
                                               UInt32           unit )
{
...
void SuperVIAATADriver::programTimingRegisters( void )
{
    if (fProvider->getHardwareType() != VIA_HW_SATA)
    {
...
void SuperVIAATADriver::writeTimingIntervalNS( VIATimingReg reg,
                                               UInt32       unit,
                                               UInt32       timeNS )
{
...
void SuperVIAATADriver::writeTimingRegister( VIATimingReg reg,
                                             UInt32       unit,
                                             UInt8        periods )
{
...
UInt32 SuperVIAATADriver::readTimingIntervalNS( VIATimingReg reg, UInt32 unit )
{
    UInt32 time;
...
UInt8 SuperVIAATADriver::readTimingRegister( VIATimingReg reg, UInt32 unit )
{
...
void SuperVIAATADriver::initializeHardware( void )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
...
void SuperVIAATADriver::selectIOTiming( ataUnitID unit )
{
...
IOReturn SuperVIAATADriver::handleQueueFlush( void )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
...
IOReturn SuperVIAATADriver::message( UInt32      type,
                                      IOService * provider,
                                      void *      argument )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
...
bool SuperVIAATADriver::setDriveProperty( UInt32       driveUnit,
                                          const char * key,
                                          UInt32       value,
                                          UInt32       numberOfBits)
{
...
IOReturn SuperVIAATADriver::createChannelCommands( void )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
...
//
	...
	while ( bytesRemaining )
	{
		...
		if ( ( DMAStatus != kIOReturnSuccess ) || ( numSegmentos != 1 ) || ( elSegmento.fLength == 0 ) )
		{
			...
			panic ( "SuperVIAATA::createChannelCommands [%d] status %x segs %d phys %x:%x \n", __LINE__, (unsigned int)DMAStatus, (int)numSegmentos, (unsigned int)elSegmento.fIOVMAddr, (unsigned int)elSegmento.fLength );
		    break;
		...
		////////////////Buggy!!!!///////////////////////
		////////////////////////////////////////////////
        if (xferDataPtr + 0x01 ) // Aangepast voor x64, if geinversed
        {
        }
		else
		{
...
//            IOLog("%s: DMA buffer length %ld is odd\n",
  //                getName(), xferCount);
        ...
        starting64KBlock = xferDataPtr + 0xffff0000; // Aangepast voor x64
        ptr2EndData  = xferDataPtr + xferCount; // Aangepast voor x64
    ...
    if (index == 0)
    {
        IOLog("%s: rejected command with zero PRD count (0x%lx bytes)\n",
              getName(), (uintptr_t)_currentCommand->getByteCount());
...
bool SuperVIAATADriver::allocDMAChannel( void )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
...
bool SuperVIAATADriver::freeDMAChannel( void )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
    if ( _prdBuffer )
    {
...
void SuperVIAATADriver::initATADMAChains( PRD * descPtr )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
    UInt32 i;
...
void SuperVIAATADriver::initForPM( IOService * provider )
{
	FEDE_LOG("FEDE - %s: %s( %p)\n", getName(), __FUNCTION__, this);
...
IOReturn SuperVIAATADriver::setPowerState( unsigned long stateIndex,
...
void SuperVIAATADriver::dumpVIARegisters( void )
{
